...
### JS获取宽高
* dom.clientHeight (content + padding) 相较于父
* **dom.offsetHeight** 最佳 （content + padding + border）相较于页面边缘
...
### class extend 继承 与 ES5 的区别
#### 原型链的不同
比如说 class A extend B，那么
```js
...
```
#### new 一个实例的不同
extend 在生成实例时，因为子类自己的this对象，必须先通过父类的构造函数完成塑造，得到与父类同样的实例属性和方法，然后再对其进行加工，加上子类自己的实例属性和方法。如果不调用super方法，子类就得不到this对象。
...
### 从输入 url 到展示的过程
1. DNS 解析 域名 -> ip
这一步的工作是解析DNS获取正确的IP地址，解析域名的第一步是查找浏览器缓存，再查找本机硬盘HOST文件是否有对应的IP地址，如果本机硬盘HOST文件内没有，再查看路由器缓存，如果都没有，那么浏览器会发送一个DNS请求到ISP （ISP 是互联网服务提供商，中国的电信联通等）的DNS服务器，本地DNS服务器会首先查找自己的缓存记录，如果没有将继续发送请求至DNS根服务器，再往下逐级查找到正确的IP地址。
...
* 回流(reflow): 当元素的**尺寸、结构或触发某些属性**时，浏览器会**重新渲染页面**，称为回流。此时，浏览器需要重新经过计算，计算后还需要重新页面布局，因此是较重的操作。会触发回流的操作:
    ...
    * 查询某些属性或调用某些方法
        ...
        * scrollTo()
> 现代浏览器大多都是通过队列机制来批量更新布局，浏览器会把修改操作放在队列中，至少一个浏览器刷新（即16.6ms）才会清空队列，当你获取布局信息的时候，队列中可能有会影响这些属性或方法返回值的操作，即使没有，浏览器也会强制清空队列，触发回流与重绘来确保返回正确的值。
#### 最佳实践:
减少重绘和回流
...
token 的认证方式类似于临时的证书签名, 并且是一种服务端无状态的认证方式, 非常适合于 REST API 的场景. 所谓无状态就是**服务端并不会保存身份认证相关的数据**。
...
cookie：登陆后后端生成一个sessionid放在cookie中返回给客户端，并且服务端**一直记录着这个sessionid**，客户端以后每次请求都会带上这个sessionid，服务端通过这个sessionid来验证身份之类的操作。所以别人拿到了cookie拿到了sessionid后，就可以完全替代你。
...
XMLHttpRequest.setRequestHeader(header, value) //是设置HTTP请求头部的方法。此方法必须在  open() 方法和 send()   之间调用。
...
* 解决办法
    ...
    * 在请求地址中添加takon验证
        ...
        * 服务器先在页面埋入特定标志，在用户请求时带上此标记，来判断请求是否合法
