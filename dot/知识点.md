...
5. 服务器处理请求并返回 HTTP 报文
    ...
    * 响应头 包含响应报文的附加信息，由 名/值 对组成 （set-cookie access-control-allow-origin/method cache-control）
        * content-type: text/html text/xml text/plain image/jpeg; charset=utf-8
    * 响应体 响应返回数据
...
2. 服务器回应，发送信息 （确认使用的加密通信协议版本，一个**随机数**用域生成对话密钥，确认**加密方法**，保存公钥的服务器证书）
...
3. 从操作系统中取出颁发者CA 的公钥，然后对服务器发来的证书里面的签名进行解密，得到摘要（digest,证书内容的hash值），据此验证证书的合法性。
4. 如果存在任何一个内置证书的公钥解出的签名内容和浏览器本身根据证书内容计算出的值相同,则认定该证书是有效的.
