...
通过 Vue的 mixin方法，为beforeCreate钩子函数 设定 vuexInit 方法
...
为每一个vue实例 挂在 $store ，在创建 Vue实例时传入store 保存在 option 属性上
...
* 遍历 每一个模块的 modules 属性，建立他们直接的父子关系
...
store 的 state 属性，在访问store中的 state 时就会触发以下的 getter方法
```js
get state () {
...
### registerModule
1. module-collection.register  建立模块间的父子关系
